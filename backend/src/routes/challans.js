...
const { getFinancialYearCode, round2 } = require('../services/businessRules');
...
router.post('/', authenticate, authorize('ADMIN', 'MANAGER', 'OPERATOR'), async (req, res) => {
  try {
    ...
    const year = getFinancialYearCode(req.body.challanDate ? new Date(req.body.challanDate) : new Date());
    ...
    const challan = await prisma.challan.create({
      data: {
        ...
        quantityCum: req.body.quantityCum ? round2(req.body.quantityCum) : null,
