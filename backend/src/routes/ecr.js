...
const { getFinancialYearCode, calculateHoldDays, isPocOwner, normalizeOwnerCode, round2 } = require('../services/businessRules');

const router = express.Router();

// Helper: generate ECR number ER/YY/NNNNN
async function generateEcrNumber(forDate = new Date()) {
...
router.post('/', authenticate, authorize('ADMIN', 'MANAGER', 'OPERATOR'), async (req, res) => {
  try {
    ...
    if (!cylinder) {
      return res.status(400).json({ error: 'Cylinder not found' });
    }
    ...
    if (!holding) {
      return res.status(400).json({ error: 'No matching issue found for this customer and cylinder' });
    }
    ...
    if (returnDate < new Date(issueDate)) {
      return res.status(400).json({ error: 'Return date cannot be before issue date' });
    }

    holdDays = calculateHoldDays(issueDate, returnDate);
    ...
    const ecrNumber = await generateEcrNumber(returnDate);
    const ecr = await prisma.ecrRecord.create({
      data: {
        ...
        quantityCum: quantityCum ? round2(quantityCum) : null,
      },
    });
