...
export default function DebitNotePage() {
  ...
  const saveMut = useMutation({
    ...
    onError: (e) => toast.error(e.response?.data?.error || "Failed to save debit note"),
  });
  ...
  return (
    <div className="space-y-4" data-testid="debit-note-page">
      <h1 className="text-2xl font-bold text-slate-900" style={{ fontFamily: "var(--font-heading)" }}>Debit Note</h1>

      <Card className="border border-slate-200 shadow-sm">
        <CardHeader className="pb-3">
          <CardTitle className="text-lg" style={{ fontFamily: "var(--font-heading)" }}>Debit Note Entry</CardTitle>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              <div>
                ...
                <Select value={form.partyCode} onValueChange={(v) => setForm({ ...form, partyCode: v })}>
                  <SelectTrigger className="h-9 mt-1" data-testid="debit-note-party-select"><SelectValue placeholder="Select party" /></SelectTrigger>
                  <SelectContent>
            ...
            <div className="flex justify-end">
              <Button type="submit" className="h-9 bg-blue-600 hover:bg-blue-700" disabled={saveMut.isPending}>
                {saveMut.isPending ? "Saving..." : "Save Debit Note"}
              </Button>
            </div>
          </form>
        </CardContent>
